...
class SingleGroupSnapshotPageWidget extends StatefulWidget {
  ...
  SingleGroupSnapshotPageWidget(
      {required this.snapshotId, required this.groupId});

  @override
  SingleGroupSnapshotPageState createState() {
    return new SingleGroupSnapshotPageState(
        snapshotId: snapshotId, groupId: groupId);
  }
}

class SingleGroupSnapshotPageState
    extends State<SingleGroupSnapshotPageWidget> {
  final int snapshotId;
  final int groupId;

  GroupSnapshotInfoResponse groupSnapshot = new GroupSnapshotInfoResponse(
    snapshotId: 1,
    snapshotType: 'd',
    auditType: '1',
    beginMoment: 'e',
      ...
      try {
        ...
        if (responseContent.result) {
          ...
          setState(() {
            ...
            var participantsKpis = List<GroupParticipantKPIResponse>.from(
                groupSnapshot.participantsKPIS
                    .map((item) => GroupParticipantKPIResponse.fromJson(item)));

            histogramData = participantsKpis
                .map((item) => new ParticipantKPIModel(
                    participantName: item.participantName,
                    kpi: item.participantKPI))
                .toList();
            ...
            leading: IconButton(
              ...
              onPressed: () {
                Navigator.pushReplacement(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          GroupManagerPageWidget(groupId: groupId)),
                );
              },
            ),
          ),
              ...
              child: Column(
                ...
                children: !isServerDataLoaded
                    ...
                    : [
                        ...
                        Text(
                          aliaser.GetAlias(aliaser.getSnapshotTypeEnumValue(
                              groupSnapshot.snapshotType)),
                          style:
                              TextStyle(fontSize: 16, color: Colors.deepPurple),
                        ),
                        SizedBox(height: 8.0),
                        ...
                        Text(
                          aliaser.GetAlias(aliaser
                              .getAuditTypeEnumValue(groupSnapshot.auditType)),
                          style:
                              TextStyle(fontSize: 16, color: Colors.deepPurple),
                        ),
                        SizedBox(height: 8.0),
                        ...
                        Text(
                          utf8.decode(
                              utf8.encode(groupSnapshot.averageKPI.toString())),
                          style:
                              TextStyle(fontSize: 16, color: Colors.deepPurple),
                        ),
                        SizedBox(height: 12.0),
                        ...
                        !isDiagramMode
                            ...
                            : SfCartesianChart(
                                ...
                                series: <CartesianSeries>[
                                    StackedColumnSeries<ParticipantKPIModel,
                                        String>(
                                      dataSource: histogramData,
                                      xValueMapper:
                                          (ParticipantKPIModel model, _) =>
...
class ParticipantKPIModel {
  ParticipantKPIModel({required this.participantName, required this.kpi});

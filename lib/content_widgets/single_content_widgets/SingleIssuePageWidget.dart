...
class SingleIssuePageState extends State<SingleIssuePageWidget> {
  ...
  Future<void> getExistedIssue(BuildContext context) async {
    ...
    if (cachedData != null) {
      ...
      var model =
          new IssueInfoRequest(userId: userId, token: token, issueId: issueId);
  ...
  Widget build(BuildContext context) {
    ...
    return new MaterialApp(
        ...
        home: Scaffold(
          appBar: AppBar(
            title: Text(
              'Страничка просмотра запроса для администрации',
                    ...
                    : [
                        ...
                        isSelfUser
                            ? TextFormField(
                                ...
                                decoration: InputDecoration(
                                    labelText:
                                        'Ссылка на скриншот (или страницу): ',
                                    labelStyle: TextStyle(
                                        fontSize: 16, color: Colors.deepPurple),
                            ...
                            : Text(
                                'Ссылка на скриншот: ' +
                                    issueLinkController.text,
                                style: TextStyle(
                                    fontSize: 16, color: Colors.deepPurple),
                              ),
                        SizedBox(height: 12.0),
                        Image.network(
                            utf8.decode(utf8.encode(issueLinkController.text)),
                            scale: 0.01),
                        SizedBox(height: 30.0),
                        ElevatedButton(
                          onPressed: () async {
                            setState(() {
                              ...
                              isLinkValidated =
                                  !issueLinkController.text.isEmpty;
